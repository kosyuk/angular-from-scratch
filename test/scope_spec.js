...
describe("Scope", function(){
	...
	describe("digest", function(){
		...
		it("executes $evalAsync'ed function later in the same cycle", function() {
			...
			scope.$watch(
				...
				function(newValue, oldValue, scope) {
					...
					scope.asynvEvaluatedImmediately = scope.asyncEvaluated;
				}
			);
		...
		it("evantually halts $evalAsyncs added by watches", function() {
			scope.aValue = [1, 2, 3];
		...
		it("does not inclue $$postDigest in the digest", function() {
		...
		it("allows a $watch to destroy another diring digest", function() {
			scope.aValue = 'abc';
			scope.counter = 0;
...
//not brokes 
	...
	describe('$watchGroup', function() {
		...
		it('calls the elistener once when the watch array is empty', function(){
			var gotNewValues, gotOldValues;
	...
	describe("inheritance", function() {
		...
		it("doesn't shadow members of parent scope's attribute", function() {
			var parent = new Scope();
			var child = parent.$new();
	...
	describe("$watchCollection", function() {
		...
		it("notices whan the value becomes an array", function() {
			scope.counter = 0;
		...
		it("notices whan item added to an array", function() {
			scope.arr = [1, 2, 3];
			scope.counter = 0;
		...
		it("notices whan item removed from an array", function() {
			scope.arr = [1, 2, 3];
			scope.counter = 0;
		...
		it("notices when an attribute is changed from an object", function() {
			scope.counter = 0;
			scope.obj = {a: 1};
	...
	describe("Events", function() {
		...
		it("allows refistering listeners", function() {
		...
		_.forEach(['$emit', '$broadcast'], function(method) {
			...
			it("is sets default Prevented when preventDefault called on "+method, function() {
			...
			it("does not stop on exception on "+method, function() {
		...
		it("sets currentScope tu null after propagation on $emit", function(){
			var event;
		...
		it("sets currentScope tu null after propagation on $broadcast", function(){
			var event;
			var scopeListener = function(evt) {
				event = evt;
			};
		...
		if('accepts one-time watches', function(){
			var theValue;
			scope.aValue = 42;
			scope.$watch('::aValue', function(newValue, oldValue, scope) {
				theValue = newValue;
		...
		it('suports short-circuiting in OR expressions', function(){
			var invoked = false;
