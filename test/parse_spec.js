...
describe("parse", function(){
	...
	it ("makes integers both constant and litereal", function() {
		var fn = parse('42');
	...
	it ("can parse a floating point number wihout an integer part", function() {
		var fn = parse('.42');
		expect(fn()).toBe(0.42);
	});

	it ("can parse a floating point in scientific notation", function() {
		var fn = parse('42e3');
	...
	it ("will not parse a string with mistatching quotes", function() {
	...
	it ("will not parse a string with invalid unicode escapes", function() {
		expect(function(){ parse('"\\uu00T0"'); }).toThrow();
	});	
	...
	it ("looks up 2-part identifier path from the scope", function() {
	...
	it ("uses locals when a 2-part keys matches in locals", function() {
		var fn = parse('aKey.anotherKey');
		expect(fn(
			{aKey: {anotherKey: 42}}, 
			{aKey: {anotherKey: 43}}
	...
	it ("does not use localse when a 2-part key does not match", function() {
		var fn = parse('aKey.anotherKey');
		expect(fn(
			{aKey: {anotherKey: 42}}, 
	...
	it ('uses locals when there is a mathching local 4-part key', function() {
	...
	it('uses locals when there is the first part in the local key', function() {
		var fn = parse('aKey.key2.key4.key4');
		expect(fn(
			{aKey:{key2: {key3: {key4: 42 }}}},
			{aKey:{}}
			)).toBeUndefined();
	...
	it('does not use locals when there is no mathcing 4-part key', function() {
		var fn = parse('aKey.key2.key3.key4');
		expect(fn(
			{aKey:{key2: {key3: {key4: 42 }}}},
			{otherKey:{anotherKey: 43}}
	...
	it('parses a propery access with another key as property', function() {
		var fn = parse('lock[key]');
		expect(fn({key:'theKey', lock:{theKey: 42}})).toBe(42);
	});

	it('parses propery access with another access as property', function() {
	...
	it('makes object constant when they only contain constants', function(){
		var fn = parse('{a: 1, b: {c: 3}}');
		expect(fn.constant).toBe(true);
	});

	it('makes object non-constant when they contain non-constants', function(){
	...
	it('returns the functionitself when given one', function() {
		var fn = function(){};
